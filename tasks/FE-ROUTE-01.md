...
- 入口改为 router 驱动：个人中心入口实际位于 **TopHeader 头像下拉**（Sidebar 无 profile 导航项）——下拉四个子项由 `navigateTo('user_profile',{profileTab})` 改为 `navigate('/profile?tab=...')`；`ProfilePage` 解析 `?tab=` 并同步遗留 `userProfileTab`，保留子 tab 定位。
...
- **EH2 入口一致**：profile 入口在 TopHeader 头像下拉，迁移后下拉四个子项改为 `navigate('/profile?tab=...')`（URL 驱动）；`ProfilePage` 用 `?tab=` 同步遗留 `userProfileTab`，避免「从下拉进是 MainLayout/currentTab、直接访问 /profile 却是 AppShell」的双态分裂。
- **EH3 回滚**：改动收敛于 AppShell + AppRouter + ProfilePage + TopHeader；`git revert` 单 commit 可回滚；历史 `/profile`（LegacyPageWrapper 分支）在本 commit 中被替换但不删除文件，FE-CONTEXT-REMOVE 阶段才删除。
- **EH4 与 FE-QUERY-01 解耦**：本 task 不碰数据层；ProfilePage 仅做路由层壳。已知留白：/profile 是真实路由后 TopHeader 面包屑不再依赖 currentTab 显示「个人中心」（过渡期接受，路由全量迁移后由面包屑改为 URL 驱动解决）。

## Scope

- 新增 `src/app/AppShell.tsx`（Sidebar + TopHeader + Outlet + Modals + Toast）。
- 新增 `src/features/profile/ProfilePage.tsx`（解析 `?tab=` + 委托渲染 `UserProfileView`）。
- 修改 `src/router/AppRouter.tsx`：`/profile` 真实路由（AppShell 嵌套路由）。
- 修改 `src/components/layout/TopHeader.tsx`：头像下拉子项改 router 跳转（`navigate('/profile?tab=...')`）。
- 更新 `src/test/router.test.tsx`：`/profile` 断言改为「直接渲染 UserProfileView + AppShell 壳」；新增 `?tab=preferences` 子 tab 断言。
...
- 修改：`frontend-jobcraft/src/components/layout/TopHeader.tsx`（头像下拉 → `/profile?tab=`）
...
- 直接访问 `/profile`：渲染 `AppShell` 壳 + `UserProfileView`（不依赖 LegacyPageWrapper），刷新后不跳回工作台。
- TopHeader 头像下拉四个子项跳转到 `/profile?tab={profile|preferences|resumes|settings}`，目标子 tab 被打开（`?tab=` 生效）。
