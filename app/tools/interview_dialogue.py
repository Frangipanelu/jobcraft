"""
面试对话解析层：把原始面试记录文本解析为结构化对话片段，并配对 QA 对。

职责：
1. 解析多种来源（手工录入、语音转文字等）的面试文本
2. 识别/推断说话人角色（面试官 / 候选人）
3. 把面试官问题与候选人回答配对为 QA 对

本模块为纯函数，不触发任何 LLM 调用，不访问数据库。
由 app.tools.interview_review 的业务层调用，并向外 re-export。
"""
