...
def ndcg_at_k(predicted: list[str], gold_ranking: list[str], gold_labels: dict[str, str], k: int = 3) -> float:
    gold_pos = {item: i for i, item in enumerate(gold_ranking)}
    predicted = predicted[:k]
    rel = [max(0, len(gold_ranking) - gold_pos.get(item, len(gold_ranking))) for item in predicted]
    ideal = sorted(
        [max(0, len(gold_ranking) - gold_pos.get(item, len(gold_ranking))) for item in gold_ranking],
        reverse=True,
    )[:k]
...
def evaluate(gold_cases: list[dict[str, Any]], predictions: list[dict[str, Any]]) -> dict[str, float]:
    ...
    for case in gold_cases:
        ...
        for exp_id, label in gold["relevance"].items():
            ...
            score_errors.append(abs(float(gold["score"].get(exp_id, 0)) - float(item.get("score", 0))))
        ranking = [x["experience_id"] for x in sorted(pred_items, key=lambda x: float(x.get("score", 0)), reverse=True)]
